---
url: /blog/kekn0l70/index.md
---
## 子区间前缀和总和的第 k 小值

### 问题描述

给定一个长度为 $n$ 的整数序列 $a\_1, a\_2, \dots, a\_n$。定义函数 $f(l, r)$ 如下：

$$
f(l, r) = \sum\_{i=l}^{r} \left( \sum\_{t=l}^{i} a\_t \right)
$$
...
* 每组测试数据描述如下：
  ...
  * 第二行输入 $n$ 个整数 $a\_1, \dots, a\_n$（$0 \le a\_i \le 10^6$）。
...
令 $S\_i = \sum\_{t=1}^i a\_t$ 为前缀和，则 $\sum\_{t=l}^i a\_t = S\_i - S\_{l-1}$，可得：

$$
f(l, r) = \sum\_{i=l}^r (S\_i - S\_{l-1}) = \left( \sum\_{i=l}^r S\_i \right) - (r - l + 1) S\_{l-1}
$$

令 $SS\_i = \sum\_{j=1}^i S\_j$ 为前缀和的前缀和，带入 $f(l, r)$：

$$
f(l, r) = (SS\_r - SS\_{l-1}) - (r - l + 1) S\_{l-1}
$$
...
由于 $a\_i \ge 0$，随着区间长度增加或包含的元素变大，$f(l, r)$ 的值具有单调性。因此，我们**二分枚举第 $k$ 小的值 $X$**。
